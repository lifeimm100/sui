---
name: cherry-pick
description: Use when a user wants to cherry-pick a commit from main to a Sui release branch and open a PR against that release branch. Do not use for general git merges, rebases, or cherry-picks that are not targeting a release branch.
---
...
```
Use the `cherry-pick` skill to cherry-pick <commit-sha> to release <release-version>.
```

Example: `Use the cherry-pick skill to cherry-pick abc123 to release 1.64.`

## Inputs

The user's request should contain two values:
1. The commit SHA to cherry-pick
2. The release version number (e.g., "1.64" or "1.65")

## Instructions

Parse the user's request to extract the commit SHA and release version. If either value is missing or unclear, ask the user for clarification.
