...
description: Use when modifying or reviewing changes in `crates/sui-protocol-config/src/lib.rs` so protocol-version changes stay safe, released versions are not edited, and required snapshots are updated. Do not use for unrelated configuration or feature work outside Sui protocol config.
---
...
```
Use the `protocol-config` skill to review or modify `crates/sui-protocol-config/src/lib.rs`.
```
