---
name: simtest-debug
description: Use when a user needs to reproduce and debug a failing `cargo simtest` with logging and hypothesis-driven experiments. Do not use for general unit test failures or for implementing a fix before the root cause is understood.
---
...
```
Use the `simtest-debug` skill to debug <repro command or test>.
```

Example: `Use the simtest-debug skill to debug MSIM_TEST_SEED=1768248386016 RUST_LOG=sui=debug,info cargo simtest --test address_balance_tests test_deposit_and_withdraw`
Example: `Use the simtest-debug skill to debug test_deposit_and_withdraw`

## Inputs

The user's request should contain either:
...
1. Commit only the logging changes (code modifications) with a message indicating the experiment number: `Codex: experiment N logging`
2. Make a separate commit for NOTEBOOK.md updates with a message like `Codex: experiment N observations`
...
An "experiment" consists of adding logging statments to the code which can confirm or refute the hypothesis. All logging statements should be of the form `info!("Codex: ...")` so that you can grep for them easily.
